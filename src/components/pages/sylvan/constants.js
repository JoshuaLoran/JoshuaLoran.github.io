import weeblyIcon from "@iconify/icons-logos/weebly";
import mailchimpFreddie from "@iconify/icons-logos/mailchimp-freddie";
import nodejsIcon from "@iconify/icons-logos/nodejs";
import microsoftWindows from "@iconify/icons-logos/microsoft-windows";

// Blurb about Sylvan Heights
export const sylvanBlurb = "Sylvan Heights is a wonderful condominium community nestled in the West Hills, 10 minutes by" +
  " car from downtown Portland. Originally built as apartments and converted to condos in 1979, the complex includes 272" +
  " residential units, with a shared recreation center, gym, spa, pool and tennis courts. The steep hillside location is" +
  " peaceful and filled with songbirds, yet convenient to downtown Portland.";

////////////////////////////////////
// Content for the Sylvan worktree
////////////////////////////////////
export const workTreeData = {
  topHeading: "Responsibilities",
  topList: [
    "Update website with news and monthly content",
    "Maintain and track financial and legal documents both public and private",
    "Design and send out email campaigns according to board guidelines for all or certain subdivisions",
    "Maintain mailchimp database of constantly revolving tenants",
    "Wrote a script to make CRUD calls to the Mailchimp API to easily apply resident changes or modify their account"
  ],
  secondHeading: "Tools",
  secondList: [
    "Weebly",
    "Mailchimp",
    "Word",
    "Javascript",
    "Quickbooks"
  ],
  linkUrl: "https://www.sylvanheightscondo.com/"
};

// Tech icons used in the Sylvan Heights project
export const sylvanIcons = [
  {
    icon: weeblyIcon,
    link: "https://www.weebly.com/"
  },
  {
    icon: mailchimpFreddie,
    link: "https://mailchimp.com/"
  },
  {
    icon: microsoftWindows,
    link: "https://www.office.com/"
  },
  {
    icon: nodejsIcon,
    link: "https://nodejs.org/en/"
  }
];
